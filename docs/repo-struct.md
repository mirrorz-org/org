...
- mirrorz-302 (redirect service)
  - * Loads site and endpoint configuration from mirrorz-config/sites/*/config.json
...
- mirrorz-monitor (updates the influxdb database)
  - config                                 --> mirrorz-config
- mirrorz-config (shared frontend, parser, and redirect configuration under config and sites)
```
